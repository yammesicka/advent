...
def next_valid_steps(xy: complex, slope: int) -> list[complex]:
    return [
        xy + step for step in DIRECTIONS
...
def peaks(xy: complex, slope: int) -> set[complex]:
    if GRID[xy] == 9:
        return {xy}
    return set().union(*(peaks(next_xy, slope + 1) for next_xy in next_valid_steps(xy, slope)))
